...
## 8. Three things that broke, and what they taught me
...
**The dashboard's own embed snippet didn't actually connect.** Lua Admin generates `window.LuaPop.init()` with no arguments, described as pulling agent config from the dashboard's record for the whitelisted domain. Loading it and inspecting `window.LuaPop.config` afterward showed `agentId: ""` and `environment: "staging"` — it never made a network call at all, let alone authenticated one. This agent only exists on `production` (never deployed to staging), so even the "correct" snippet couldn't have worked as shipped. Passing `agentId` and `environment: "production"` explicitly made the widget render, connect, and hold a real conversation — confirmed by driving an actual browser against the served page, not by reading the widget's source. Worth knowing if the dashboard's copy-paste snippet ever silently fails again: check `window.LuaPop.config` before assuming the domain whitelist is the problem.
